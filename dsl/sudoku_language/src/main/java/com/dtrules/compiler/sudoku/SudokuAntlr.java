...
/**
 * ANTLR 4 based Sudoku Language Compiler implementation.
 * This replaces the JFlex/CUP based SudokuLanguage compiler with ANTLR 4.
 */
public class SudokuAntlr implements ICompiler {
    ...
    private HashMap<String, SudokuTypeResolver.RLocalType> localtypes = new HashMap<String, SudokuTypeResolver.RLocalType>();
    ...
    private void addType(IREntity entity, RName name, int itype) throws Exception {
        SLType type = (SLType) types.get(name);
        if (type == null) {
            type = new SLType(name, itype, entity);
            types.put(name, type);
        } else {
    ...
    public HashMap<RName, IRType> getTypes(EntityFactory ef) throws Exception {
        ...
        while (tables.hasNext()) {
            ...
            SLType type = new SLType(tablename, IRObject.iDecisiontable, (REntity) ef.getDecisiontables());
    ...
    public void printTypes(PrintStream out) throws RulesException {
        ...
        for (int i = 0; i < typenames.length - 1; i++) {
            for (int j = 0; j < typenames.length - 1; j++) {
                RName one = (RName) typenames[j], two = (RName) typenames[j + 1];
                if (((SLType) types.get(one)).getType() > ((SLType) types.get(two)).getType()) {
                    Object hold = typenames[j];
                    typenames[j] = typenames[j + 1];
                    typenames[j + 1] = hold;
                }
     ...
     */
    private static class SudokuErrorListener extends BaseErrorListener {
        private final String source;
        private RulesException error;

        public SudokuErrorListener(String source) {
            this.source = source;
        }
    ...
    private String compile(String s) throws RulesException {
        ...
        SudokuLexer lexer = new SudokuLexer(CharStreams.fromString(s));
        CommonTokenStream tokens = new CommonTokenStream(lexer);
        SudokuParser parser = new SudokuParser(tokens);

        // Set up error handling
        SudokuErrorListener errorListener = new SudokuErrorListener(s);
        ...
        // Parse
        SudokuParser.DoneContext tree = parser.done();
        ...
        SudokuTypeResolver typeResolver = new SudokuTypeResolver(session, types, localtypes);
        SudokuCompilerVisitor visitor = new SudokuCompilerVisitor(typeResolver);
    ...
    public ArrayList<String> getPossibleReferenced() {
        ...
        for (IRType type : types.values()) {
            v.addAll(((SLType) type).getPossibleReferenced());
        }
        return v;
    }
