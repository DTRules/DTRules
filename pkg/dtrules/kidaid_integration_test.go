...
// TestKidAidIntegration tests the full pipeline with the KidAid sample project.
// This mirrors the Java TestKidAid class which loads the EDD, decision tables,
// and mapping, then executes Compute_Eligibility against test case data.
func TestKidAidIntegration(t *testing.T) {
	kidAidDir := findKidAidDir(t)
	if kidAidDir == "" {
		t.Skip("KidAid sample project not found")
	}

	t.Logf("Using KidAid directory: %s", kidAidDir)

	// Create a new RuleSet
	rs := session.NewRuleSet("KidAid")

	// Load the EDD
	eddPath := filepath.Join(kidAidDir, "repository/xml/kidaid_edd.xml")
	...
	dtPath := filepath.Join(kidAidDir, "repository/xml/kidaid_dt.xml")
	...
	// Load the mapping
	mapPath := filepath.Join(kidAidDir, "repository/xml/kidaid_map.xml")
	...
	// Get the main decision table (matches Java TestKidAid.getDecisionTableName())
	...
	// Check results on the job entity (mirrors Java printReport which reads job.results)
...
// TestKidAidIntegrationTestCase002 runs the pipeline with TestCase_002 data
// (pregnant 19-year-old mother) to exercise a different eligibility path.
func TestKidAidIntegrationTestCase002(t *testing.T) {
	kidAidDir := findKidAidDir(t)
	if kidAidDir == "" {
		t.Skip("KidAid sample project not found")
	}
...
// TestKidAidEDDLoad tests just the EDD loading for KidAid
func TestKidAidEDDLoad(t *testing.T) {
	kidAidDir := findKidAidDir(t)
	if kidAidDir == "" {
		t.Skip("KidAid sample project not found")
	}
...
// TestKidAidDTLoad tests decision table loading and structure
func TestKidAidDTLoad(t *testing.T) {
	kidAidDir := findKidAidDir(t)
	if kidAidDir == "" {
		t.Skip("KidAid sample project not found")
	}
...
// TestKidAidMappingLoad tests mapping loading and data loading
func TestKidAidMappingLoad(t *testing.T) {
	kidAidDir := findKidAidDir(t)
	if kidAidDir == "" {
		t.Skip("KidAid sample project not found")
	}
...
// findKidAidDir locates the KidAid sample project directory
func findKidAidDir(t *testing.T) string {
	// Try relative paths from the test location
	paths := []string{
		"../../../../sampleprojects/KidAid",
		"../../../sampleprojects/KidAid",
		"../../sampleprojects/KidAid",
		"../sampleprojects/KidAid",
		"sampleprojects/KidAid",
	...
	knownPaths := []string{
		filepath.Join(home, "DTRules/sampleprojects/KidAid"),
		filepath.Join(home, "repos/github.com/DTRules/DTRules/sampleprojects/KidAid"),
		"/home/paul/DTRules/sampleprojects/KidAid",
	}
