...
// TestManifestEnforcementBlocksOverwrite verifies that the manifest prevents
// AI from overwriting user changes to Excel. This is the critical test for
// "Excel is source of truth" enforcement.
func TestManifestEnforcementBlocksOverwrite(t *testing.T) {
	tmpDir := t.TempDir()
	xmlDir := filepath.Join(tmpDir, "xml")
	excelDir := filepath.Join(tmpDir, "excel")
	...
	// Step 1: AI creates XML and exports to Excel
	xmlDT := filepath.Join(xmlDir, "rules_dt.xml")
	excelDT := filepath.Join(excelDir, "rules_dt.xlsx")

	if err := os.WriteFile(xmlDT, []byte("<dt>AI VERSION 1</dt>"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	// Step 2: User modifies Excel (simulating user edit)
	time.Sleep(100 * time.Millisecond)
	futureTime := time.Now().Add(2 * time.Second)
	if err := os.WriteFile(excelDT, []byte("USER MODIFIED"), 0644); err != nil {
		t.Fatal(err)
	}
	if err := os.Chtimes(excelDT, futureTime, futureTime); err != nil {
		t.Fatal(err)
	}

	// Step 3: AI modifies XML and tries to export again
	if err := os.WriteFile(xmlDT, []byte("<dt>AI VERSION 2</dt>"), 0644); err != nil {
		t.Fatal(err)
	}

	// Reload manifest to pick up the recorded export
	...
	if err == nil {
		t.Fatal("CRITICAL: Export should be REJECTED when Excel was modified by user")
	}
	...
	t.Logf("SUCCESS: Manifest enforcement blocked AI from overwriting user changes: %v", err)
}
