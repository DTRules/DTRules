...
**Semantics**: Returns an integer, signed from `d1` to `d2`. Postfix operators: `daysbetween`, `monthsbetween`, `yearsbetween`. `days from` counts calendar days with both ends included (#1265): the dates from `d1`'s date to `d2`'s date, both read in `d1`'s zone. Mar 8 to Mar 10 is 3, a date to itself is 1, Jan 1 to Dec 31 is 365, and Mar 10 back to Mar 8 is -3; the result is never 0. The time of day does not count (Mar 8 23:00 to Mar 9 01:00 is 2; Mar 8 01:00 to Mar 8 23:00 is 1), and neither does a daylight-saving change (America/Chicago, 2026-03-08 00:00 to 2026-03-10 00:00 is 47 hours and 3 days). So `days from d to d + N days` is N + 1 (N - 1 for negative N); do not add 1 for an inclusive count. `months from` compares year and month; `years from` counts anniversaries; each reads a date's fields in the zone that date carries.
