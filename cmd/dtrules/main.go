...
func main() {
	...
	if len(os.Args) > 1 {
		cmd := os.Args[1]
		// Check for subcommands (don't start with -)
		if !strings.HasPrefix(cmd, "-") && subcommands[cmd] {
			cli := NewCLI()
	...
	flag.Usage = func() {
		...
		fmt.Fprintf(os.Stderr, "  %s sync status                    # Show sync status\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  %s sync check                     # Check for pending user edits\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  %s sync import                    # Import Excel to XML\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  %s sync export                    # Export XML to Excel\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  %s init my-rules                  # Initialize new project\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "\nLegacy Examples:\n")
		fmt.Fprintf(os.Stderr, "  %s -rules ./rules -list\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  %s -rules ./rules -validate\n", os.Args[0])
		fmt.Fprintf(os.Stderr, "  %s -rules ./rules -entry Main\n", os.Args[0])
	}
